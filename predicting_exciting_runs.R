...
model_data = cbind(cbind(cbind(model_data, OFFENSIVE_RUN),TEAM),model.matrix( ~ PERIOD-1, data=possess_log_16_17))
...
model_data = model_data[, c(8,10,11,12,13,15,16,21)] # choose a subset of variables
...
formula2 = as.formula(paste("OFFENSIVE_RUN ~", paste(paste(names(model_data),sep=""), collapse="+")))
formula2
...
run_sum_3 = summary(step(run_pred_3, direction = "backward", k = log(32), trace = 0))
run_sum_3


# show some boxplots for coefficients for various models:
