...
possession1617 <- read.delim("~/NBA/NBA_TrackingData_2014-17/2016-17_nba_possession_log.txt", row.names=NULL, stringsAsFactors=FALSE)
...
possession1617.dataset = possession1617.game.label %>% select(GAME_ID,PERIOD,TEAM,AWAY_PTS,AWAY_PTS_NET,STREAK.6.120.DIREC,STREAK.TEAM,PASSES,TOUCHES,DRIBBLES) %>% group_by(GAME_ID) %>% mutate(RESPONSE = lead(STREAK.6.120.DIREC,default = 0),AWAY_PTS_ROLL3=roll_sum_fill(AWAY_PTS,3),PERIOD.FAC = as.factor(PERIOD)) %>% ungroup() %>% select(GAME_ID,PERIOD.FAC,TEAM,AWAY_PTS_NET,STREAK.TEAM,PASSES,TOUCHES,DRIBBLES,RESPONSE,AWAY_PTS_ROLL3)
...
possession1617.dataset.test.truth = possession1617.dataset %>% filter(GAME_ID %in% game.test) %>% select(RESPONSE)
...
With this multinomial model, we can achieve a ~58% one-step prediction accuracy and a ~86% top2 prediction accuracy.

# Visualize multinomial model results
```{r visualize multinomial}
#possession1617.pred <- as.data.frame(possession1617.pred)
#names(possession1617.pred) <- c("Away_Run","No_Run","Home_Run")
#ggplot(data=possession1617.pred)+geom_line(aes(y=))
#plot(possession1617.pred[,1],type="l")

#game_mark = game.test[6]

#game_mark = game.test[23]

game_mark = game.test[42]
...
game_selected_test = game_selected %>% select(GAME_ID,PERIOD,TEAM,AWAY_PTS,AWAY_PTS_NET,STREAK.6.120.DIREC,STREAK.TEAM,PASSES,TOUCHES,DRIBBLES,TIME_ELAPSED)  %>% mutate(RESPONSE = lead(STREAK.6.120.DIREC,default = 0),AWAY_PTS_ROLL3=roll_sum_fill(AWAY_PTS,3),PERIOD.FAC = as.factor(PERIOD))

game_selected_test_forpred = game_selected_test %>% select(PERIOD.FAC,TEAM,AWAY_PTS_NET,AWAY_PTS_ROLL3,STREAK.TEAM,PASSES,TOUCHES,DRIBBLES)

#game_selected_pred = predict(possession1617.fit,game_selected_test_forpred)

game_selected_test$PRED = lag(predict(possession1617.fit,game_selected_test_forpred),default = as.factor(0))

game_selected_pred_awayrun = game_selected_test %>% filter(PRED == -1)
game_selected_pred_homerun = game_selected_test %>% filter(PRED == 1)

ggplot()+geom_line(data=game_selected,mapping = aes(x=TIME_ELAPSED,y=AWAY_PTS_NET,color=STREAK.6.120+1),show.legend = F,size=0.8)+geom_vline(xintercept = game_selected_timeout$TIME_ELAPSED,color=(game_selected_timeout$TEAM+2)*2,linetype=2,size=0.5)+geom_point(data=game_selected_pred_awayrun,mapping = aes(x=TIME_ELAPSED,y=AWAY_PTS_NET),color="orange",show.legend = F)+geom_point(data=game_selected_pred_homerun,mapping = aes(x=TIME_ELAPSED,y=AWAY_PTS_NET),color="darkgreen",show.legend = F)+labs(x="seconds in game",y="net points won")+ggtitle(paste("Game_ID:",game_mark))

```

# Try a simple neural network with same dataset
```{r try neural net}
library(neuralnet)
vars <- names(possession1617.dataset.train)
f1 <- as.formula(paste(paste("~", paste(vars, collapse = " + ")),"-1"))
possession1617.dataset.train.matrix <- model.matrix(f1,data=possession1617.dataset.train)

vars <- colnames(possession1617.dataset.train.matrix)
f <- as.formula(paste("RESPONSE ~", paste(vars[!vars %in% "RESPONSE"], collapse = " + ")))

possession1617.nn <- neuralnet(f,data=possession1617.dataset.train.matrix,hidden=c(16,8),linear.output=F)
plot(possession1617.nn)
```
